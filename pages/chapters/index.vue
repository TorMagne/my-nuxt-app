        ...
        <form @submit.prevent="submitForm">
          <label class="form-control w-full max-w-xs mb-4">
            ...
            <select class="select select-bordered" v-model="form.book" required>
              <option disabled selected value="" class="hidden">Select one</option>
...
const filteredChapters = computed(() => {
  ...
  return chapters.value.body.filter(
    (chapter) =>
      chapter.book.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      chapter.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      chapter.level.toString().includes(searchQuery.value.toLowerCase()) ||
